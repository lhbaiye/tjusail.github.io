...
def get_data():
    path_list = dict()
    for dirname, _, filenames in os.walk('D:/code/regression/dataset/winequality'):
        for filename in filenames:
            filename, file_type = os.path.join(dirname, filename), filename.split('.')[0]
            path_list[file_type] = filename
    # 训练集
    data = pd.read_csv(path_list['winequality-red'])
    # 数据处理
    reviews = []
...
def load_model(version=0, drop_col_number=1):
    model = Sequential()
    if version != 0:
        model.add(Dense(16, activation='relu', input_dim=11-drop_col_number))
...
def main(params):
    ...
    for dirname, _, filenames in os.walk('D:/code/regression/dataset/winequality'):
        for filename in filenames:
            ...
            path_list[file_type] = filename
    # 训练集
    train_df = pd.read_csv(path_list['winequality-red'])
